...
import type { TorontoGuide } from "@/data/toronto-guides";
import { TORONTO_GUIDE_LINKS } from "@/data/toronto-guides";
import { getExpediaTravelShopOutbound } from "@/lib/affiliate";
import { GlassPanel } from "@/components/ui/glass-panel";

export function TorontoNeighbourhoodGuide({ guide }: { guide: TorontoGuide }) {
  ...
  return (
    <article className="min-h-screen bg-zinc-950">
      <header className="relative overflow-hidden border-b border-white/10">
        <Image
          src={guide.heroImage}
          alt={`${guide.name} — Toronto FIFA 2026 fan travel`}
          fill
          priority
          sizes="100vw"
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-12 sm:px-6">
        ...
        <GlassPanel>
          <h2 className="font-display text-2xl text-white">{guide.bmoFieldConnection.headline}</h2>
          <div className="mt-5 overflow-x-auto">
            <table className="w-full text-sm">
              <tbody>
                {guide.bmoFieldConnection.rows.map((row, i) => (
                  <tr
                    key={row.label}
          ...
          {guide.slug !== "bmo-field" ? (
            <p className="mt-4 text-sm text-zinc-500">
              Stadium details:{" "}
              <Link href="/guides/toronto/bmo-field" className="font-semibold text-cyan-400 hover:text-cyan-300">
                BMO Field matchday guide →
              </Link>
            </p>
          ) : null}
        </GlassPanel>
        ...
        <GlassPanel>
          <h2 className="font-display text-2xl text-white">More Toronto guides</h2>
          <ul className="mt-4 flex flex-wrap gap-3">
            {TORONTO_GUIDE_LINKS.filter((l) => !l.href.endsWith(guide.slug)).map((link) => (
              <li key={link.href}>
                <Link
                  href={link.href}
        ...
        <GlassPanel className="text-center sm:text-left">
          <div className="sm:flex sm:items-center sm:justify-between sm:gap-8">
            <div>
              <h2 className="font-display text-3xl text-white">Book your Toronto stay</h2>
              <p className="mt-2 max-w-xl text-sm text-zinc-400">
                Compare FIFA World Cup 2026™ hotel deals near {guide.name} and across Toronto.
              </p>
            </div>
            <div className="mt-6 flex shrink-0 flex-col gap-3 sm:mt-0">
              <a
                ...
                className="inline-flex min-h-12 items-center justify-center rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 px-8 text-sm font-bold text-zinc-950 shadow-lg shadow-emerald-500/25 transition hover:scale-[1.02] hover:brightness-110"
              >
